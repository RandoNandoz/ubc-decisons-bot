...
async def send_applicant_to_review(applicant_decision: ApplicantDecision):
    ...
    class ReviewView(discord.ui.View):
        ...
        async def reject(self, button: discord.ui.Button, interaction: discord.Interaction):
            unreviewed_decisions.delete_one({"discord_id": applicant_decision.discord_id})
            await interaction.response.send_message(f"Rejected, make sure to PM why <@{applicant_decision.discord_id}>", ephemeral=True)
            await interaction.message.delete()
...
async def decision(
    ...
    try:
        ...
        applicant_decision = ApplicantDecision(
        ...
        await ctx.respond("Success! Your decision has been submitted for review by the mods. Expect a response within 24 hours")
    except ValueError:
